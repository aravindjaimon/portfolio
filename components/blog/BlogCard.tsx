...
export function BlogCard({ post, featured = false }: BlogCardProps) {
  return (
    ...
    >
      <Link href={post.permalink} className="block relative aspect-video overflow-hidden">
        <Image
          src={post.coverImage}
          alt={post.title}
          fill
      ...
      <div className={`p-4 sm:p-6 flex flex-col ${featured ? 'justify-center' : ''}`}>
        ...
        <p className="text-white/60 text-sm sm:text-base mt-2 mb-4 line-clamp-2">{post.description}</p>
        ...
        <div className="flex flex-wrap gap-2 mt-auto mb-4">
          ...
          {post.tags.length > 3 && (
            <span className="text-xs text-white/40 font-mono self-center">+{post.tags.length - 3}</span>
          )}
        </div>
