...
function createHeading(level: 1 | 2 | 3 | 4 | 5 | 6) {
  ...
  return function Heading({ children, id }: { children: React.ReactNode; id?: string }) {
    const headingId = id || (typeof children === 'string' ? children.toLowerCase().replace(/\s+/g, '-') : undefined);

    return (
      <Tag id={headingId} className={`font-bebas text-white tracking-wide group ${sizes[level]}`}>
        <a href={`#${headingId}`} className="inline-flex items-center gap-2 hover:text-[#C41E3A] transition-colors">
          {children}
          <LinkIcon size={level <= 2 ? 20 : 16} className="opacity-0 group-hover:opacity-100 transition-opacity" />
        </a>
      </Tag>
    );
  };
...
// Code block wrapper
function Pre({ children, ...props }: React.HTMLAttributes<HTMLPreElement> & { raw?: string }) {
...
function MDXImage({ src, alt }: { src?: string; alt?: string }) {
  ...
  return (
    <figure className="my-8">
      ...
      {alt && <figcaption className="mt-2 text-center text-sm text-white/50 font-mono">{alt}</figcaption>}
    </figure>
  );
}
...
function Table({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6 overflow-x-auto">
      <table className="w-full border-collapse border border-[#2D2D2D]">{children}</table>
    </div>
  );
}
...
function Td({ children }: { children: React.ReactNode }) {
  return (
    <td className="border border-[#2D2D2D] px-4 py-2 text-sm text-white/70">{children}</td>
  );
}
...
function Ul({ children }: { children: React.ReactNode }) {
  return <ul className="my-4 ml-6 list-disc text-white/70 space-y-2">{children}</ul>;
}

function Ol({ children }: { children: React.ReactNode }) {
  return <ol className="my-4 ml-6 list-decimal text-white/70 space-y-2">{children}</ol>;
}
